...
#         data = struct.unpack('>BffffH', temp)
...
class EchoLocator(object):
    ...
    def __init__(self, hydrophone_locations, wave_propagation_speed):  # speed in m/s
        ...
        #     severity=2
        # )
        self.hydrophone_locations = np.array([[0, 0, 0], [-0.0254, 0, 0.0254], [0.0254, 0, 0], [0, -0.0254, 0]])
    ...
    def getPulseLocation(self, timestamps):
        ...
        if(result.success):
            return result.x[:3], -np.sqrt(np.sum(np.square(result.x))) / self.wave_propagation_speed
        else:
            print "error"
            return None
...
def testFileHex(filename):
    '''
    Runs the trilateration algorithm on timestamps written to a file in the following format:
    [ ref_tsdatatamp tstamp1 tstamp2 tstamp3 ]
    lines that do not begin with '[' are ignored
    '''
    locator = EchoLocator(None, 1484)
    positions = []
    with open(filename, "r") as data_file:
        for line in data_file:
            b = binascii.a2b_hex(line[:38])
            data = struct.unpack('>BffffH', b)
            real_data = np.array([data[4], data[1], data[2], data[3]])
            positions.append(locator.getPulseLocation(real_data))
    for p in positions:
        print p
